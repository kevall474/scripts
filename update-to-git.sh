...
git clone https://github.com/kevall474/llvm-pkg.git && cd llvm-pkg && sed -i '93s/ninja/ninja -j12/' clang-git/PKGBUILD && sed -i 's/ninja -C build all/ninja -j12 -C build all/' lib32-llvm-git/PKGBUILD && chmod +x build.sh && ./build.sh && cd ..
